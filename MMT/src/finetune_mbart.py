...
# This script fine tune for the Hausa language multimodal machine translation task.
# The prerequisite is the extracted object tags concatnated with the train/dev/test set.
# Change the GENERATE_ONLY flag for training and evaluation
...
TOKENIZER = M2M100Tokenizer.from_pretrained("facebook/m2m100_418M", src_lang="en", tgt_lang="ha")
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm_meter):
            ...
            generated_tokens = model.generate(
                **inputs,
                forced_bos_token_id=TOKENIZER.lang_code_to_id[f"ha"],
                num_beams=beam_size,
            )
...
def train_till_convergence(
        ...
        dev_references: List[str], test_references: List[str],
    ...
    for split_name, iterator, references in zip(['dev', 'test'],
                                                [dev_iter, test_iter],
                                                [dev_references, test_references]):
