...
if __name__ == "__main__":
    ...
    hausa_vqa_image_order_paths = {
        'train': os.path.join('../data/raw/HaVQA/hausa-train.txt'),
        'dev': os.path.join('../data/raw/HaVQA/hausa-dev.txt'),
        'test': os.path.join('../data/raw/HaVQA/hausa-test.txt'),
    }
    obj_tags_dir = os.path.join('../data/object_tags')
    for target_lang, folder_name in zip(['ha'], ['HaVQA']):
        ...
        hausa_vqa_data_paths = {split: os.path.join(prepared_data_dir, f'{split}.en.{target_lang}.tsv') for split in
                                    ['train', 'dev', 'test']}
        for split, path in hausa_vqa_data_paths.items():
            append_object_tags(
                data_df=pd.read_csv(path, sep='\t', encoding='utf-8', na_filter=False),
                image_ids=pd.read_csv(hausa_vqa_image_order_paths[split], encoding='utf-8', sep='\t', header=None)[
                    0].to_list(),
